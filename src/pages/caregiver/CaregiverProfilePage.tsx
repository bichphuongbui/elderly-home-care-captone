...
const CaregiverProfilePage: React.FC = () => {
  ...
  // Form states
  const [personalInfo, setPersonalInfo] = useState<Required<NonNullable<CaregiverProfile['personalInfo']>>>({
    dateOfBirth: '',
    gender: '',
    idNumber: '',
    permanentAddress: '',
  ...
  const [professionalInfo, setProfessionalInfo] = useState<Required<NonNullable<CaregiverProfile['professionalInfo']>>>({
    yearsOfExperience: 0,
    previousWorkplace: '',
    skills: '',
    languages: [],
  ...
  const [references, setReferences] = useState<Required<NonNullable<CaregiverProfile['references']>>>({
    referenceName: '',
    referencePhone: '',
    referenceRelation: ''
  });

  const [commitments, setCommitments] = useState<Required<NonNullable<CaregiverProfile['commitments']>>>({
    ethicalCommitment: false,
    termsAgreement: false
  });

  const [additionalProfile, setAdditionalProfile] = useState<Required<NonNullable<CaregiverProfile['additionalProfile']>>>({
    profilePhoto: '',
    introductionVideo: '',
    specialAbilities: ''
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get<UserData>(`https://68aed258b91dfcdd62ba657c.mockapi.io/users/${userId}`);
        ...
        if (profile.legalDocuments) {
          setFilePreviews(prev => ({
            ...
            vaccinationCertificate: profile.legalDocuments?.vaccinationCertificate || '',
        ...
        if (profile.professionalInfo?.certificateFiles) {
          ...
          if (certificateFiles.length > 0) {
            ...
            if (isOldFormat) {
              ...
              const convertedCertificates: CertificateFile[] = (certificateFiles as string[]).map((url, index) => ({
                id: `legacy-${index}`,
                url: url,
              ...
              const approvedCertificates = convertedCertificates.filter(cert => cert.status === 'approved');
              const pendingCertificates = convertedCertificates.filter(cert => cert.status === 'pending');
              const rejectedCertificates = convertedCertificates.filter(cert => cert.status === 'rejected');
              ...
              const newFormatCertificates = certificateFiles as CertificateFile[];
              const approvedCertificates = newFormatCertificates.filter(cert => cert.status === 'approved');
              const pendingCertificates = newFormatCertificates.filter(cert => cert.status === 'pending');
              const rejectedCertificates = newFormatCertificates.filter(cert => cert.status === 'rejected');
              setCertificatePreviews(approvedCertificates);
              setPendingCertificates(pendingCertificates);
  ...
  const validators = {
    ...
    phone: (v: string) => (!/^(0[3|5|7|8|9])[0-9]{8}$|^(\+84[3|5|7|8|9])[0-9]{8}$/.test(v) ? 'Số điện thoại VN không hợp lệ' : ''),
    permanentAddress: (v: string) => (!v || v.trim().length < 5 ? 'Địa chỉ thường trú tối thiểu 5 ký tự' : ''),
    yearsOfExperience: (v: number) => (v < 0 || v > 50 ? 'Số năm kinh nghiệm 0-50' : '')
  } as const;
  ...
  const onFieldBlur = (field: keyof typeof validators, value: any) => {
    ...
    setErrors(prev => {
      ...
      if (err) next[field as string] = err; else delete next[field as string];
      return next;
    });
  };

  const handleFileUpload = async (field: keyof typeof filePreviews, file: File) => {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        if (field === 'criminalRecord') {
          ...
          setLegalDocuments(prev => ({ ...prev, healthCertificate: dataUrl }));
        } else if (field === 'vaccinationCertificate') {
          setLegalDocuments(prev => ({ ...prev, vaccinationCertificate: dataUrl }));
  ...
  const handleSave = async () => {
    ...
    if (personalInfo.permanentAddress) {
      const addressError = validators.permanentAddress(personalInfo.permanentAddress);
      if (addressError) criticalErrors.permanentAddress = addressError;
    }
    
    if (professionalInfo.yearsOfExperience !== undefined && professionalInfo.yearsOfExperience !== null) {
      const expError = validators.yearsOfExperience(professionalInfo.yearsOfExperience);
    ...
    if (Object.keys(criticalErrors).length > 0) {
      setMessage('Có một số lỗi format, nhưng vẫn có thể lưu. Vui lòng kiểm tra lại thông tin.');
    }
    ...
    try {
      ...
      const apiData = {
        ...
        profile: {
          ...
          professionalInfo: {
            ...
            certificateFiles: updatedProfile.professionalInfo?.certificateFiles || []
          }
        }
      };
      ...
      let response;
      try {
        response = await axios.put(`https://68aed258b91dfcdd62ba657c.mockapi.io/users/${userId}`, apiData);
        console.log('Save response:', response.data);
      } catch (apiError: any) {
        console.log('First attempt failed, trying with simplified data structure...');
        ...
        const simplifiedData = {
          ...
          profile: {
            ...
            professionalInfo: {
              ...
              certificateFiles: (updatedProfile.professionalInfo?.certificateFiles as CertificateFile[])?.map(cert => ({
                id: cert.id,
                url: cert.url,
                status: cert.status,
        ...
        response = await axios.put(`https://68aed258b91dfcdd62ba657c.mockapi.io/users/${userId}`, simplifiedData);
        console.log('Fallback save response:', response.data);
      }
      
      // Check if there are pending certificates and notify admin
      const certificateFiles = updatedProfile.professionalInfo?.certificateFiles as CertificateFile[] | undefined;
      const hasPendingCertificates = certificateFiles?.some(cert => cert.status === 'pending');
      const pendingCount = certificateFiles?.filter(cert => cert.status === 'pending').length || 0;
      ...
      if (hasPendingCertificates) {
        ...
        setMessage('Lưu thay đổi thành công. Admin đã được thông báo về chứng chỉ mới cần duyệt.');
      } else {
      ...
      if (e.response?.status === 400) {
        ...
        setMessage(`Có lỗi xảy ra khi lưu thay đổi: ${e.response?.data?.message || e.message}`);
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <section className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-6 text-center">Ảnh đại diện</h2>
        <div className="flex flex-col items-center space-y-4">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-xs text-gray-500">Chấp nhận: JPG, PNG (tối đa 5MB)</p>
      ...
      {/* Basic info - readonly */}
      <section className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">1. Thông tin cơ bản</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Họ tên</label>
            <input type="text" readOnly value={user?.fullName || ''} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-500" />
          </div>
          <div>
      ...
      {/* Personal & contact */}
      <section className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">2. Thông tin cá nhân & liên hệ</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className={`mt-1 block w-full px-3 py-2 border rounded-md ${errors.dateOfBirth ? 'border-red-500' : 'border-gray-300'}`}
              max={(() => { const d = new Date(); d.setFullYear(d.getFullYear() - 18); return d.toISOString().split('T')[0]; })()}
              min={(() => { const d = new Date(); d.setFullYear(d.getFullYear() - 65); return d.toISOString().split('T')[0]; })()}
              ref={!errors.dateOfBirth ? undefined : (el) => { if (!firstErrorRef.current) firstErrorRef.current = el; }}
            />
          ...
          <div>
            ...
            <select
              ...
              className={`mt-1 block w-full px-3 py-2 border rounded-md ${errors.gender ? 'border-red-500' : 'border-gray-300'}`}
              ref={!errors.gender ? undefined : (el) => { if (!firstErrorRef.current) firstErrorRef.current = el; }}
            >
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Số CCCD không thể thay đổi sau khi đăng ký</p>
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setPersonalInfo(prev => ({ ...prev, phone: e.target.value.replace(/[^\d+]/g, '').slice(0, 12) }))}
              onBlur={(e) => onFieldBlur('phone', e.target.value)}
              className={`mt-1 block w-full px-3 py-2 border rounded-md ${errors.phone ? 'border-red-500' : 'border-gray-300'}`}
              placeholder="0123456789 hoặc +84123456789"
              ref={!errors.phone ? undefined : (el) => { if (!firstErrorRef.current) firstErrorRef.current = el; }}
            />
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className={`mt-1 block w-full px-3 py-2 border rounded-md ${errors.permanentAddress ? 'border-red-500' : 'border-gray-300'}`}
              rows={3}
              ref={!errors.permanentAddress ? undefined : (el) => { if (!firstErrorRef.current) firstErrorRef.current = el as any; }}
            />
          ...
          <div className="md:col-span-2">
            ...
            <textarea value={personalInfo.temporaryAddress} onChange={(e) => setPersonalInfo(prev => ({ ...prev, temporaryAddress: e.target.value }))} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" rows={3} />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                {filePreviews.idCardFront ? (
                  <div className="mt-3">
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-2">Ảnh đã được tải lên khi đăng ký</p>
                  </div>
                ) : (
                  <div className="h-32 w-full bg-gray-100 border-2 border-dashed border-gray-300 rounded-md flex items-center justify-center">
                    <span className="text-gray-500 text-sm">Chưa có ảnh CCCD mặt trước</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {filePreviews.idCardBack ? (
                  ...
                  <div className="h-32 w-full bg-gray-100 border-2 border-dashed border-gray-300 rounded-md flex items-center justify-center">
                    <span className="text-gray-500 text-sm">Chưa có ảnh CCCD mặt sau</span>
                  </div>
                )}
              </div>
            </div>
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
              <p className="text-blue-800 text-sm">
                <strong>Lưu ý:</strong> Ảnh CCCD không thể thay đổi sau khi đăng ký. Nếu cần cập nhật, vui lòng liên hệ quản trị viên.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Professional */}
      <section className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">3. Thông tin nghề nghiệp</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setProfessionalInfo(prev => ({ ...prev, yearsOfExperience: Number(e.target.value) || 0 }))}
              onBlur={(e) => onFieldBlur('yearsOfExperience', Number(e.target.value) || 0)}
              className={`mt-1 block w-full px-3 py-2 border rounded-md ${errors.yearsOfExperience ? 'border-red-500' : 'border-gray-300'}`}
              ref={!errors.yearsOfExperience ? undefined : (el) => { if (!firstErrorRef.current) firstErrorRef.current = el; }}
            />
            {errors.yearsOfExperience && <p className="text-red-500 text-xs mt-1">{errors.yearsOfExperience}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Nơi từng làm việc</label>
          ...
          <div className="md:col-span-2">
            ...
            {certificatePreviews.length > 0 && (
              <div className="mt-3">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Chứng chỉ đã được duyệt:</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {certificatePreviews.map((certificate) => (
                    <div key={certificate.id} className="flex items-center justify-between p-2 border border-green-200 rounded-md bg-green-50">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-green-600">
                            Duyệt lúc: {new Date(certificate.approvedAt || '').toLocaleDateString('vi-VN')}
                          </span>
                        </div>
                      </div>
                      {/* No edit/delete here */}
              ...
              <div className="mt-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {pendingCertificates.map((certificate) => (
                    <div key={certificate.id} className="flex items-center justify-between p-2 border border-yellow-200 rounded-md bg-yellow-50">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-yellow-600">
                            Upload lúc: {new Date(certificate.uploadedAt).toLocaleDateString('vi-VN')}
                          </span>
                        </div>
                      </div>
                      {/* No remove here */}
                ...
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-md">
                  <p className="text-blue-800 text-xs">
                    <strong>Lưu ý:</strong> Chứng chỉ đang chờ admin duyệt. Bạn sẽ được thông báo khi có kết quả.
                  </p>
                </div>
              </div>
            )}
      ...
      <section className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700">Khả năng đặc biệt</label>
            <textarea
      ...
      <div className="flex justify-end">
        <button
          ...
          className={`px-6 py-2 rounded text-white font-semibold ${(saving || !isValid) ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
        >
